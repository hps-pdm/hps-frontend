...
export default function AccTrendPlot({
  data,
  thresholds = null,
}: {
  data: HistoryPoint[];
  ...
  if (!data || data.length === 0) {
    return (
      <div style={{ fontSize: 12, color: "#9ca3af" }}>
        No acceleration RMS data available.
      </div>
    );
  }
  ...
  return (
    <Plot
      ...
      layout={{
        ...
        shapes,
        annotations: [
          // Y-axis title annotation removed per request
        ],
        legend: {
          orientation: "h",
          x: 0,
